...
import { POOL_ADDRESS, SOLANA_SUPPLY, TOKEN_DECIMALS } from "@/config/const"
   ...
   useEffect(() => {
      ...
      const fetchState = async () => {
         try {
            ...
            const progressPct =
               (Number(poolState.quoteReserve.toString()) / Number(configState.migrationQuoteThreshold.toString())) *
               100

            const baseRemaining = Number(poolState.baseReserve.toString()) / 10 ** TOKEN_DECIMALS
