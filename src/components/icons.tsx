...
export function SolanaIcon({ className = "w-5 h-5" }: { className?: string }) {
   return (
      <svg className={className} viewBox="0 0 397.7 311.7" fill="none" aria-hidden="true">
         <path
...
/**
 * @dev USDC icon - public/usdc.svg
 */
export function USDCIcon({ className = "w-5 h-5" }: { className?: string }) {
   return <img src="/usdc.svg" alt="USDC" className={className} />
}

/**
 * @dev USDT icon - public/usdt.svg
 */
export function USDTIcon({ className = "w-5 h-5" }: { className?: string }) {
   return <img src="/usdt.svg" alt="USDT" className={className} />
}

/**
 * @dev Maps a token mint to its icon component
 */
export function TokenIcon({ mint, className = "w-5 h-5" }: { mint: string; className?: string }) {
   switch (mint) {
      case SOL_MINT:
