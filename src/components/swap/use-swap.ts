...
export function useSwap() {
  ...
  const getQuote = useCallback(
    async (amountIn: BN, inputMint: string, outputMint: string, slippageBps: number) => {
      setError(null);
      ...
      try {
        ...
        return await getJupiterQuote(amountIn, inputMint, outputMint, slippageBps);
      } catch (e) {
  ...
  const executeSwap = useCallback(
    async (amountIn: BN, inputMint: string, _outputMint: string, currentQuote: SwapQuote) => {
